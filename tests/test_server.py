...
class TestOdooInfrastructureSaasClientServerSlowStatistic(
    ...
    def test_01_controller_odoo_infrastructure_server_slow_statistic(self):
        ...
        data = response.json()
        self.assertTrue(isinstance(data['used_disk_space'], int))
        self.assertTrue(isinstance(data['free_disk_space'], int))
        self.assertTrue(isinstance(data['total_disk_space'], int))
